...
`.artifacts/` is gitignored scratch space for ephemeral HTML files generated by an agent to help **the human reader** quickly grasp something visually — PR explainers, design explorations, research summaries, planning visuals, custom one-shot editors. Full convention in `docs/workflow/artifacts.md`.

**Decision rule:**

- **Markdown** when the audience is the agent (specs, workflow, governance, this file, CHANGELOG) — grep-able, diff-able, low token cost.
- **HTML in `.artifacts/`** when the audience is the human reader and the content is one-shot visual (side-by-side comparison, diagrams, interactive prototype). Ephemeral, gitignored, never imported by code.

Never put durable contracts in `.artifacts/`. Never commit `.artifacts/` content. In-app user-facing help (Tauri webview) is **not** an artifact — it is production code with separate concerns.
