...
export function SettingsScreen({ state, onSave, onReset, onRetry }: SettingsScreenProps) {
  if (state.status === "loading") {
    return (
      <div className="flex h-full items-center justify-center text-[13px] text-[var(--color-fg-3)]">
        Loading settings…
      </div>
    );
  }
  ...
  if (state.status === "error") {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3">
        ...
        <div className="text-[11.5px] text-[var(--color-fg-3)]">{state.error.detail}</div>
          ...
          <ul className="divide-y divide-[var(--color-divider-soft)]">
            <li className="px-4 py-3.5 flex items-center gap-4">
              ...
              <input
                ...
                className="w-44 rounded-md border border-[var(--color-divider-soft)] bg-[var(--color-surface-2)] px-2.5 py-1.5 text-[13px] outline-none focus:border-[var(--color-fg-2)]"
              />
            </li>
            <li className="px-4 py-3.5 flex items-center gap-4">
              <div className="flex-1">
                ...
                <div className="text-[11.5px] text-[var(--color-fg-3)] mt-0.5">
                  Undo logs older than this are cleared at startup. Min {MIN_RETENTION_DAYS}, max{" "}
                  {MAX_RETENTION_DAYS}.
                </div>
              </div>
              <input
                ...
                className="w-24 rounded-md border border-[var(--color-divider-soft)] bg-[var(--color-surface-2)] px-2.5 py-1.5 text-[13px] outline-none focus:border-[var(--color-fg-2)]"
              />
            </li>
            <li className="px-4 py-3.5 flex items-center gap-4">
              <div className="flex-1">
              ...
              <div className="text-[13px] text-[var(--color-fg-3)]">{languageDisplay}</div>
            </li>
          </ul>
        </Card>
