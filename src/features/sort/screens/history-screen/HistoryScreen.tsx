      ...
      <div className="flex-1 overflow-y-auto px-7 py-7">
        <Card className="overflow-hidden">
          <ul className="divide-y divide-[var(--color-divider-soft)]">
            {state.items.map((job) => (
              <HistoryRow key={job.id} job={job} nowMs={referenceNowMs} onRevert={onRevert} />
            ))}
          </ul>
        </Card>
      </div>
...
function HistoryRow({ job, nowMs, onRevert }: HistoryRowProps) {
  ...
  return (
    <li className="px-4 py-3 flex items-center gap-4 hover:bg-[var(--color-hover-soft)]">
...
function HistoryStats({ job }: { job: HistoryItemDto }) {
  return (
    <div className="font-mono text-[11.5px] text-[var(--color-fg-2)] tabular-nums whitespace-nowrap">
...
function Centered({ children }: { children: React.ReactNode }) {
  return (
    <div className="h-full flex items-center justify-center text-[13px] text-[var(--color-fg-3)] flex-col">
      {children}
    </div>
  );
}
